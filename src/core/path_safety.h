...
[[nodiscard]] inline bool ReadFileBytesNoFollowLimitedWin32(
    ...
    const bool metadataOk =
        GetFileInformationByHandleEx(handle, FileAttributeTagInfo, &tagInfo, sizeof(tagInfo)) &&
        (tagInfo.FileAttributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT)) == 0 &&
        GetFileSizeEx(handle, &size) && size.QuadPart >= 0 &&
        static_cast<std::uint64_t>(size.QuadPart) <= maxBytes &&
    ...
    while (total < out.size()) {
        const DWORD requested = static_cast<DWORD>(
            std::min<size_t>(out.size() - total, static_cast<size_t>(1 << 20)));
        DWORD read = 0;
        if (!ReadFile(handle, out.data() + total, requested, &read, nullptr) || read == 0) {
            out.clear();
            CloseHandle(handle);
            return false;
        }
