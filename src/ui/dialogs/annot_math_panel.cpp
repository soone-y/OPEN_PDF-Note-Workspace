...
bool InspectorHasFont(const Annotation::Type type) {
    return type == Annotation::Type::TextBox;
}
...
void SetAnnotInspectorEditable(HWND hWnd, AnnotInspectorCtx* ctx, bool editable) {
    ...
    for (const int id : { kAnnotInspectorX1, kAnnotInspectorY1, kAnnotInspectorX2, kAnnotInspectorY2,
                          ...
                          kAnnotInspectorArrowHead, kAnnotInspectorDash, kAnnotInspectorFontName,
                          kAnnotInspectorFontPt, kAnnotInspectorApply }) {
...
bool ApplyAnnotInspectorAll(HWND hWnd, AnnotInspectorCtx* ctx, bool showValidation) {
    if (!ctx || ctx->initializing || !ctx->editable) return true;
    if (!ctx->annotationId.empty()) {
        for (size_t i = 0; i < g_annots.size(); ++i) {
            if (g_annots[i].id == ctx->annotationId) {
                ctx->index = static_cast<int>(i);
                break;
            }
        }
    }
    if (ctx->index < 0 || ctx->index >= static_cast<int>(g_annots.size()) ||
        (!ctx->annotationId.empty() && g_annots[static_cast<size_t>(ctx->index)].id != ctx->annotationId)) {
        if (showValidation) ShowInspectorValidation(hWnd, kAnnotInspectorApply,
                                                    L"注釈が変更または削除されたため、適用できません。");
        return false;
    }
    ...
    if (InspectorHasFont(after.type)) {
        HWND fontNameControl = GetDlgItem(hWnd, kAnnotInspectorFontName);
        const int selected = fontNameControl
            ? static_cast<int>(SendMessageW(fontNameControl, CB_GETCURSEL, 0, 0)) : CB_ERR;
        if (selected == CB_ERR) {
            if (showValidation) ShowNoSoundPopup(hWnd, kAnnotInspectorFontName, L"プロパティエラー",
                                                  L"フォントをリストから選択してください。");
            return false;
        }
        const std::wstring fontName = ResolveFontFaceName(GetComboItemText(fontNameControl, selected));
        if (fontName.empty()) return false;
        double fontPt = 0.0;
        if (!TryReadInspectorNumber(hWnd, kAnnotInspectorFontPt, 4.0, 200.0, fontPt)) {
            if (showValidation) ShowNoSoundPopup(hWnd, kAnnotInspectorFontPt, L"プロパティエラー",
                                                  L"フォントサイズは 4〜200 pt の数値で入力してください。");
            return false;
        }
        after.fontName = fontName;
        after.fontPt = fontPt;
    }
...
LRESULT CALLBACK AnnotInspectorProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        const bool hasFont = InspectorHasFont(ann.type);
        ...
        if (hasFont) {
            CreateWindowExW(0, L"BUTTON", L"文字の設定", WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
                            8, currY, 496, 62, hWnd, nullptr, g_hInst, nullptr);
            const std::wstring resolvedFontName = ann.fontName.empty() ? g_textFontName : ann.fontName;
            const double resolvedFontPt = ann.fontPt > 0.0 ? ann.fontPt : g_textFontPt;
            CreateWindowExW(0, L"STATIC", L"フォント:", WS_CHILD | WS_VISIBLE,
                            20, currY + 27, 52, 20, hWnd, nullptr, g_hInst, nullptr);
            HWND fontName = CreateWindowExW(0, L"COMBOBOX", L"",
                            WS_CHILD | WS_VISIBLE | WS_TABSTOP | CBS_DROPDOWNLIST | WS_VSCROLL,
                            74, currY + 23, 260, 24, hWnd,
                            reinterpret_cast<HMENU>(kAnnotInspectorFontName), g_hInst, nullptr);
            AppendDefaultFontListToCombo(fontName);
            const std::wstring fontDisplayName = ResolveFontDisplayName(resolvedFontName);
            int fontSelection = static_cast<int>(SendMessageW(fontName, CB_FINDSTRINGEXACT, -1,
                                                               reinterpret_cast<LPARAM>(fontDisplayName.c_str())));
            // 古い注釈に保存された候補外の書体も、内容を失わず確認・再適用できる。
            if (fontSelection == CB_ERR) {
                fontSelection = static_cast<int>(SendMessageW(fontName, CB_ADDSTRING, 0,
                                                               reinterpret_cast<LPARAM>(fontDisplayName.c_str())));
            }
            if (fontSelection != CB_ERR) SendMessageW(fontName, CB_SETCURSEL, fontSelection, 0);
            CreateWindowExW(0, L"STATIC", L"サイズ:", WS_CHILD | WS_VISIBLE,
                            342, currY + 27, 44, 20, hWnd, nullptr, g_hInst, nullptr);
            CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", InspectorNumberText(resolvedFontPt).c_str(),
                            WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
                            388, currY + 23, 58, 24, hWnd,
                            reinterpret_cast<HMENU>(kAnnotInspectorFontPt), g_hInst, nullptr);
            CreateWindowExW(0, L"STATIC", L"pt", WS_CHILD | WS_VISIBLE,
                            450, currY + 27, 20, 20, hWnd, nullptr, g_hInst, nullptr);
            currY += 62 + 10;
        }
    ...
    case WM_COMMAND: {
        ...
        if ((id == kAnnotInspectorWidth || id == kAnnotInspectorAlpha || id == kAnnotInspectorFontName ||
...
static int FindAnnotationIndexByPanelSelectionId() {
    if (s_selectedAnnotId.empty()) return -1;
    for (size_t i = 0; i < g_annots.size(); ++i) {
        if (g_annots[i].id == s_selectedAnnotId) return static_cast<int>(i);
    }
    return -1;
}

void SyncAnnotPanelSelectionFromList() {
    if (!g_hAnnotList) return;
    const int selected = static_cast<int>(SendMessageW(g_hAnnotList, LB_GETCURSEL, 0, 0));
    s_selectedAnnotId = (selected >= 0 && selected < static_cast<int>(g_annots.size()))
        ? g_annots[static_cast<size_t>(selected)].id
        : L"";
}

void ClearAnnotPanelSelection() {
    s_selectedAnnotId.clear();
    if (g_hAnnotList) SendMessageW(g_hAnnotList, LB_SETCURSEL, static_cast<WPARAM>(-1), 0);
    UpdateAnnotPanelSummary();
    if (g_hAnnotList) InvalidateRect(g_hAnnotList, nullptr, FALSE);
}

void SelectAnnotPanelAnnotationByIndex(int index, bool reveal) {
    if (index < 0 || index >= static_cast<int>(g_annots.size())) {
        ClearAnnotPanelSelection();
        return;
    }
    s_selectedAnnotId = g_annots[static_cast<size_t>(index)].id;
    if (g_hAnnotList) {
        SendMessageW(g_hAnnotList, LB_SETCURSEL, static_cast<WPARAM>(index), 0);
        if (reveal) SendMessageW(g_hAnnotList, LB_SETTOPINDEX, static_cast<WPARAM>(index), 0);
        InvalidateRect(g_hAnnotList, nullptr, FALSE);
    }
    UpdateAnnotPanelSummary();
}
...
void RefreshAnnotPanel() {
    ...
    if (s_selectedAnnotId.empty()) SyncAnnotPanelSelectionFromList();
    ...
    if (s_revealLatestAnnotOnNextRefresh && count > 0) {
        targetSel = count - 1;
        s_selectedAnnotId = g_annots.back().id;
    } else {
        targetSel = FindAnnotationIndexByPanelSelectionId();
    }
    s_revealLatestAnnotOnNextRefresh = false;
    if (targetSel >= 0) {
        SelectAnnotPanelAnnotationByIndex(targetSel, true);
    } else {
        ClearAnnotPanelSelection();
    }
    UpdateAnnotPanelSummary();
...
bool ShowAnnotPanelContextMenu(HWND owner, POINT screenPt) {
    ...
    if (screenPt.x != -1 || screenPt.y != -1) {
        ...
        SelectAnnotPanelAnnotationByIndex(index);
