...
// Context-menu "close" commands only end the current display.  They never
// remove a directory, a file, or a temporary-path registration.
static bool PrepareContextDisplayClose(HWND owner, const wchar_t* operationJa,
                                       const wchar_t* operationEn) {
    if (!SaveNoteIfDirty(owner)) {
        SyncLeftPaneSelection();
...
static void FinishContextDisplayClose(HWND owner) {
    SyncLeftPaneSelection();
    RefreshMainWindowUiState(owner);
    RefreshStatusDisplay(owner);
}

static bool CloseFileDisplayFromContext(HWND owner, bool isNote, const std::wstring& path) {
    if (path.empty()) return false;
    const std::wstring openPath = isNote ? g_currentNotePath : CurrentLogicalPdfPath();
    if (NormalizePathKeyForList(path) != NormalizePathKeyForList(openPath)) return false;
    if (!PrepareContextDisplayClose(owner, L"ファイルを閉じる", L"Closing file")) return false;
...
static bool CloseSessionDisplayFromContext(HWND owner, const std::wstring& path) {
    if (path.empty() || SessionKeyFromPath(path) != SessionKeyFromPath(g_currentSessionPath)) return false;
    if (!PrepareContextDisplayClose(owner, L"回次を閉じる", L"Closing session")) return false;

    ClearPdfAndNoteSelection();
    g_currentSessionPath.clear();
    g_pdfFiles.clear();
    g_noteFiles.clear();
    if (g_hSessionList) SendMessageW(g_hSessionList, LB_SETCURSEL, static_cast<WPARAM>(-1), 0);
    if (g_hPdfList) {
...
static bool CloseLectureDisplayFromContext(HWND owner, const std::wstring& path) {
    if (path.empty() || NormalizePathKeyForList(path) != NormalizePathKeyForList(g_currentLecturePath)) return false;
    if (!PrepareContextDisplayClose(owner, L"授業を閉じる", L"Closing lecture")) return false;
