...
int LeaveBlock(MD_BLOCKTYPE, void*, void* userdata) {
    ...
    // MD4C does not emit text callbacks for a thematic break. Its source span
    // is therefore located eagerly when entering MD_BLOCK_HR; do not replace
    // that exact line span with the cursor range, which can include preceding
    // blank lines.
    Span blockSpan = (node.kind == BlockKind::HorizontalRule && node.span.end > node.span.start)
                         ? node.span
                         : (frame.content.has_span
                                ? frame.content.span
...
void NormalizeDashThematicBreaks(NoteDocument* out, const NoteTextModel& model) {
    ...
    for (size_t index = 0; index < out->blocks.size(); ++index) {
        BlockNode& heading = out->blocks[index];
        if (heading.kind != BlockKind::Heading || heading.level != 2) continue;

        const size_t ruleStart = SkipLineBreak(raw, std::min(heading.span.end.value, raw.size()));
        size_t ruleEnd = FindNextLineBreak(raw, ruleStart);
        if (ruleEnd == std::wstring::npos) ruleEnd = raw.size();
        ...
        for (auto& block : out->blocks) {
            if (block.parent != kInvalidIndex && block.parent >= insertAt) ++block.parent;
        }
        for (auto& inlineNode : out->inlines) {
            if (inlineNode.parent_block != kInvalidIndex && inlineNode.parent_block >= insertAt) {
                ++inlineNode.parent_block;
            }
        }

        BlockNode rule;
        rule.kind = BlockKind::HorizontalRule;
        rule.origin = BlockOrigin::Markdown;
        rule.parent = heading.parent;
        rule.span = Span{{ruleStart}, {SkipLineBreak(raw, ruleEnd)}};
        rule.loc = ResolveLineColumn(model, ruleStart);
        rule.first_inline = out->inlines.size();
        rule.inline_count = 0;
        out->blocks.insert(out->blocks.begin() + static_cast<ptrdiff_t>(insertAt), std::move(rule));
        ++index;
    }
}
...
NoteDocument ParseNoteDocumentWithMd4c(const NoteTextModel& model) {
    ...
    NormalizeDashThematicBreaks(&out, model);
