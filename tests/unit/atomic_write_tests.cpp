...
static bool TestQuarantineNeverOverwritesExistingPath(const fs::path& root) {
    const fs::path sourceDir = root / L"quarantine_source";
    const fs::path quarantine = root / L"quarantine_collision";
    std::error_code ec;
    ...
    const fs::path source = sourceDir / L"recovery.txt";
    const fs::path occupied = quarantine / source.filename();
    if (!WriteRaw(source, "recovery-payload") || !WriteRaw(occupied, "do-not-overwrite")) {
        return false;
    }

    fs::path moved;
    if (!atomic_write::QuarantineFileBestEffort(source, quarantine, &moved)) return false;
    return moved != occupied &&
           ReadAll(occupied) == "do-not-overwrite" &&
           ReadAll(moved) == "recovery-payload" &&
           !fs::exists(source, ec);
}
...
int main() {
    ...
    run("Quarantine never overwrites existing path", &TestQuarantineNeverOverwritesExistingPath);
