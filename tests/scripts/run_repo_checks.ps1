...
try {
    ...
    if (-not $SkipInputFuzzRegressionTests) {
        Invoke-Step -Name "Input Fuzz Regression Tests" -Action {
            Invoke-ChildPowerShellScript -ScriptPath $inputFuzzRegressionScript
        }
    }
    ...
    if (-not $SkipDependencySecurityGate) {
        Invoke-Step -Name "Dependency Security Gate" -Action {
            Invoke-PythonScript -ScriptPath $dependencySecurityGateScript
        }
    }
    ...
    if (-not $SkipPythonToolTests) {
        Invoke-Step -Name "Python Tool Tests" -Action {
            ...
            Invoke-PythonUnittestFile -TestFile $dependencySecurityTestScript
        }
    }

    if (-not $SkipSiteValidation) {
        Invoke-Step -Name "Public Site Validation" -Action {
            Invoke-PythonScript -ScriptPath $cloudflareSiteValidationScript -Arguments @(
                "--site", "site/cloudflare/public"
            )
            Invoke-PythonScript -ScriptPath $githubSiteBuildScript -Arguments @(
                "--replace", "--documentation-portal"
            )
            Invoke-PythonScript -ScriptPath $githubSiteValidationScript -Arguments @(
                "--site", "site/github/output/public"
            )
        }
    }
