...
# zlib 1.3.2 is vendored and pinned. Keep this restricted import check as
# defense in depth because the application does not need crc32_combine* APIs.
...
foreach ($functionName in $forbiddenZlibImports) {
    ...
    if ($functionHits.Count -gt 0) {
        throw "pdf_note_workspace.exe imports unnecessary restricted zlib API: $functionName"
    }
}
$networkImportDlls = @(
    ("win" + "http.dll"),
...
$zlibLicenseDryRunHits = @($dryRun | Select-String -SimpleMatch "licenses\zlib\LICENSE")
