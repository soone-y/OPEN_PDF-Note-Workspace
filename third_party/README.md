...
- `README.md`, `CHANGELOG.md`
  - 取得元・仕様・履歴確認用の補助文書
- `VERSION`, `patches/`
  - 固定 upstream commit と、日本語向け局所変更の再現情報
...
- `VERSION` に上流 tag、commit、ビルド方法、DLL/import library の SHA-256 を固定します
- `scripts/build/build_workspace.ps1` は vendor header/import library を使い、vendor DLL を `out/bin*` へコピーします
- `scripts/release/pack_release.ps1` は toolchain の zlib へフォールバックせず、監査済み vendor DLL と `LICENSE` を同梱します
...
MinGW-w64 runtime DLL を release に同梱するためのライセンス証跡です。zlib は独立した vendor 依存として管理し、ここでは扱いません。
...
- 対応するライセンス文書は `licenses/mingw-w64/` へコピーします
