...
def recommendations_for_paths(paths: Iterable[str], diff_text: str = "") -> dict[str, list[str]]:
    ...
    input_parser_changed = (
        matches("src/clrop/")
        or matches("src/note/")
        or matches("src/pdf_view/annotation_store")
        or matches("third_party/pdfium/")
        or matches("third_party/md4c/")
    )
    if input_parser_changed:
        add_unique(run, "powershell -NoProfile -ExecutionPolicy Bypass -File tests/scripts/run_input_fuzz_regression_tests.ps1")
        add_unique(reasons, "Untrusted-input parsing changed: rerun deterministic CLROP, annotation-history, note, PDF, and image mutations.")

    dependency_security_changed = (
        matches("third_party/")
        or matches("tools/release_checks/dependency_security_")
        or matches("scripts/build/")
    )
    if dependency_security_changed:
        add_unique(run, "python tools/release_checks/dependency_security_gate.py")
