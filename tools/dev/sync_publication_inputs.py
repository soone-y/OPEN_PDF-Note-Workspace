...
def validate_publication_inputs(destination_root: Path) -> None:
    ...
    for path in destination_root.rglob("*"):
        ...
        if text.startswith("\ufeff"):
            raise ValueError(
                f"Public publication input has a duplicate UTF-8 BOM: {path.relative_to(destination_root)}"
            )
...
def sync_publication_inputs(source_root: Path, destination_root: Path) -> None:
    ...
    if not isinstance(retired_paths, list):
        raise ValueError("pages_submission.retired_paths must be an array")
    # Pages updates may replace only their explicitly managed inputs.  The
    # public repository also contains the release snapshot source tree, so
