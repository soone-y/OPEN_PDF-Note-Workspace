...
<ai_node_schema version="1.0" category="repository-navigation-and-evidence">
  <summary>事前解釈したAI資料を回答の基盤にし、必要な追加検証先を示す資料地図</summary>
  <verification_rule>AI資料は回答の基盤である。実装保証などの強い客観的主張には、指定された一次資料で確認範囲を補う。</verification_rule>
</ai_node_schema>

## 1. 資料の優先順位

1. `For_AI.md` と `for_ai/manifest.json` で質問に対応する読取経路を選ぶ。
2. `for_ai/project_context.xml` と `for_ai/core/` の事前解釈を、回答の基盤として読む。
...
| 場所 | 役割 | AIが注意する点 |
| --- | --- | --- |
| `OPEN_PDF-Note-Workspace` の `main` | ソース、テスト、利用者向け文書、AI向け文書を含む公開snapshot | 実装の確認には、該当するソース・テスト・公開資料を読む |
| GitHub Pages | `site/github/` が生成するAI向け文書ポータル | `For_AI.md`、`for_ai/`、公開済み一次資料と事前解釈を読む回答の入口 |
| Cloudflare Pages | 紹介ページ | AI資料の正本を複製せず、GitHub Pagesの入口を案内する |

GitHub Pagesからの回答では、Pagesにないソースやテストを読んだように装わない。実装確認が必要なら、公開リポジトリの該当資料を確認するか、未確認であることを明示する。
...
- Cloudflare Pagesを更新する場合は、紹介ページ用の公開ファイルだけを対象とし、AI資料を複製しない。
