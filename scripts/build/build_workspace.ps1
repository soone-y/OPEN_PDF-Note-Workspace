...
function Sync-AppRuntimeArtifacts {
    ...
    $zlibSource = "third_party/zlib/bin/zlib1.dll"
    $zlibDest = Join-Path $BinDir "zlib1.dll"
    if (-not (Test-Path -LiteralPath $zlibSource -PathType Leaf)) {
        throw "Vendored zlib runtime DLL not found: $zlibSource"
    }
    Copy-FileIfHashDiff -SourcePath $zlibSource -DestPath $zlibDest -DisplayName "zlib1.dll" | Out-Null
    $artifacts += $zlibDest
...
try {
    ...
    $libs = @(
        ...
        "third_party/zlib/lib/libz.dll.a",
        "-lgdi32",
        "-lole32",
        "-lcomdlg32",
        "-lcomctl32",
    ...
    if ($versionChanged) {
        Write-Host "Repo version changed ($appVersion); selectively recompiling version component (app_core.cpp) and relinking..." -ForegroundColor Yellow
        $appCoreObj = Join-Path $objDir "app_core.o"
        if (Test-Path -LiteralPath $appCoreObj) {
            Remove-Item -Force -LiteralPath $appCoreObj -ErrorAction SilentlyContinue
        }
        $forceLink = $true
    }
